...
public abstract class Storage {
     ...
     * @param c Class<T> used to call duplicate for template (template.duplicate(c)), not null required
     *
     * @return ArrayList<T> representing all data readed from DB, never null but could be empty if Error occurs
     */
    ...
    public static <T extends Storage> void insert(@NotNull T obj) {
        ...
        try {
            ...
            // ricerca dei vari campi "mappati" in map per il tipo T e utilizzo di updateSelected e getByName
            for (Field f : fs) {
                f.setAccessible(true);
                String n = f.getName();
                if (map.containsKey(n)) {
    ...
    private static void updateResultSet(String name) {
        try {
            ...
            System.out.printf("Error! UpdateResultSet for (%s) failed...\n", name);
        }
    }
